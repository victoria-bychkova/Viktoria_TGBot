...
bot = telebot.TeleBot(token='999token999', parse_mode='html')
# Массив с доступными расширениями файлов. Можно изменить, и добавятся/удалятся кнопки выбора на клавиатуре.
formats = ['.jpg', '.png', '.svg', '.gif', '.ico', '.mp4', '.avi', '.webm', '.doc', '.docx', '.xls', '.xlsx', '.txt', '.pdf', '.css', '.html', '.rtf', '.json', '.zip', '.rar', '.dxf', '.tiff', '.bmp', '.ppt', '.exe' ]
...
def welcome(message):
	...
	bot.send_video(message.chat.id, 'https://media1.tenor.com/m/VObrmodY2ucAAAAd/eth-blockchain.gif', None, 'Text')
	reply = bot.send_message(message.chat.id, f"Привет, <b>{username}</b>! 👋🏻\nЯ бот-генератор тестовых файлов. Помогу тебе проверить граничные значения при загрузке файлов разного веса в приложениях и на веб-сайтах. Я умею генерировать файлы различных расширений размерами от 1 байта до 49 мегабайт включительно.\n\nЧтобы начать, выбери одно из доступных расширений в меню ниже ⬇️", reply_markup=markup)
...
def check_size(message, format, unit):
	...
	# Иначе, если всё ОК и было введено корректное число, то переходим к проверке размера и генерации файла.
		...
		# Если он меньше 1 байта или больше 49 мегабайт (51380224 байт), то выводим сообщение об ошибке.
		# Иначе переходим к генерации файла.
		if (size_bytes < 1 or size_bytes > 51380224):
			reply = bot.send_message(message.chat.id, f"Размер файла выходит за границы моих возможностей.\n<u>Мои ограничения:</u>\n<b>Минимум</b> — 1 байт\n<b>Максимум</b> — 49 MB (это 46 080 KB или 51 380 224 байт)\n\nПожалуйста, введи подходящий размер 🙂")
